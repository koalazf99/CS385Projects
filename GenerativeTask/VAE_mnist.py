...
def train(args, model, trainloader, save_path):
    num_epochs = 1
    learning_rate = 1e-3
    loss_record = []
...
def test(model, testloader):
    ...
    res.save("./flow_img/MNIST_FLOW.jpg")


def comp(model, testloader, zsize):
    for batch_idx, data in enumerate(testloader):
        img, _ = data
        # img = img.view(img.size(0), -1)
        img = Variable(img)
        ...
        save = to_img(recon_batch.cpu().data)
        save_image(save, './vae_img/image_{}_{}.png'.format("REcons", str(zsize)))
        save = to_img(img.cpu().data)
        save_image(save, "./vae_img/imase_{}_{}.png".format("real", str(zsize)))
        break
...
zsize = 20
load = False
model = VAE(zsize=zsize)
if load:
    load_path = "./saved_pth/vae_mnist_zsize_{}.pth".format(str(zsize))
    checkpt = torch.load(load_path)
    model.load_state_dict(checkpt, strict=True)
if torch.cuda.is_available():
    model.cuda()

# train(None, model, dataloader, load_path)
test(model, testloader)
comp(model, testloader, zsize)
